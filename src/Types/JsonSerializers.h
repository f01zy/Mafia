...
namespace nlohmann {
template <> struct adl_serializer<Types::Player> {
  static void from_json(const json &j, Types::Player &player) {
    player.username = j.at("username").get<std::string>();
    player.role = j.at("role").get<std::string>();
  }
};
...
template <> struct adl_serializer<Types::Room> {
  static void from_json(const json &j, Types::Room &room) {
    ...
    room.players = j.at("players").get<std::vector<Types::Player>>();
